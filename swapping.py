...
class EntanglementSwappingMessage(Message):
    ...
    def __init__(self, msg_type: SwappingMsgType, receiver: str, **kwargs):
        ...
        if self.msg_type is SwappingMsgType.SWAP_RES:
            ...
            self.psi_signs = kwargs.get("psi_signs")
            self.new_psi_sign = kwargs.get("new_psi_sign")
        else:
...
class EntanglementSwappingA(EntanglementProtocol):
    ...
    def start(self) -> None:
        ...
        if self.owner.get_generator().random() < self.success_probability():
            ...
            net_psi_sign = self.left_memo.psi_sign * self.right_memo.psi_sign # 1 if same, -1 if different

            if meas_res[0] == 0:
                new_psi_sign = net_psi_sign
            else:
                new_psi_sign = -net_psi_sign
            ...
            msg_l = EntanglementSwappingMessage(SwappingMsgType.SWAP_RES,
                                                ...
                                                expire_time=expire_time, meas_res=[], psi_signs=None, new_psi_sign=new_psi_sign)  # empty meas_res
            msg_r = EntanglementSwappingMessage(SwappingMsgType.SWAP_RES, 
                                                ...
                                                expire_time=expire_time, meas_res=meas_res, psi_signs=net_psi_sign, new_psi_sign=new_psi_sign)
        else:
            # swapping failed
...
class EntanglementSwappingB(EntanglementProtocol):
    ...
    def received_message(self, src: str, msg: "EntanglementSwappingMessage") -> None:
        ...
        self.memory.psi_sign = msg.new_psi_sign # update psi_sign

        if msg.fidelity > 0 and self.owner.timeline.now() < msg.expire_time:
            # if msg.psi_signs == 1: # original EPs had same psi_sign
            #     results = msg.meas_res
            # elif msg.psi_signs == -1: # original EPs had opposite psi_signs
            #     results = [(msg.meas_res[0] + 1)%2, msg.meas_res[1]]
            # else:
            #     results = None

            if msg.meas_res == [0, 0] or msg.meas_res == [1, 0]: # either phi+ or phi- (neither bits are always one or other, depends on psi_signs)
